...
use phantomkernel_netd::NetworkBackend;
use phantomkernel_shardd::{ShardManager, ShardState, NamespaceBoundary};
...
mod tests {
    use super::*;
    use gk_audit::AuditChain;
    use phantomkernel_netd::{NetworkBackendError, RouteProfile};
    ...
    impl NamespaceBoundary for MockNamespaceBoundary {
        fn create_namespace(&self, _shard_name: &str) -> Result<(), phantomkernel_shardd::ShardError> {
            Ok(())
        }

        fn start_namespace(&self, _shard_name: &str) -> Result<(), phantomkernel_shardd::ShardError> {
            Ok(())
        }

        fn stop_namespace(&self, _shard_name: &str) -> Result<(), phantomkernel_shardd::ShardError> {
            Ok(())
        }

        fn destroy_namespace(&self, _shard_name: &str) -> Result<(), phantomkernel_shardd::ShardError> {
            Ok(())
        }
    }
