...
use phantomkernel_airlockd::{AirlockIpcHandler, AirlockService, PluggableSanitizerChain};
use gk_config::{ensure_runtime_layout, validate_runtime_layout, RuntimePaths};

fn main() {
    if let Err(error) = run() {
        eprintln!("phantomkernel-airlockd failed to start: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    println!("phantomkernel-airlockd initialized");
    Ok(())
}
