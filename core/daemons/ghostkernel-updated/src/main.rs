// PhantomKernel Updated Daemon
// A/B Update Mechanism with Rollback Support

use phantomkernel_updated::UpdateService;
use gk_audit::AuditChain;
use std::path::PathBuf;
use std::env;

fn main() {
    println!("phantomkernel-updated daemon starting...");
    
    // Determine config directories
    let config_dir = env::var("PHANTOMKERNEL_CONFIG")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("/etc/phantomkernel"));
    
    let cache_dir = env::var("PHANTOMKERNEL_CACHE")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("/var/cache/phantomkernel"));
    ...
    match UpdateService::new(config_dir.clone(), cache_dir.clone()) {
        Ok(mut service) => {
            ...
            println!("phantomkernel-updated ready");
