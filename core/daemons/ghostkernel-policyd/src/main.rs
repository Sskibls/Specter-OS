...
use phantomkernel_policyd::{CapabilityRequest, CapabilityRule, PolicyIpcHandler, PolicyService};
use gk_audit::AuditChain;
use gk_config::{ensure_runtime_layout, validate_runtime_layout, RuntimePaths};

fn main() {
    if let Err(error) = run() {
        eprintln!("phantomkernel-policyd failed to start: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    println!("phantomkernel-policyd initialized");
    Ok(())
}
