...
use phantomkernel_shardd::{LinuxNamespaceStub, ShardIpcHandler, ShardManager};
use gk_audit::AuditChain;
use gk_config::{ensure_runtime_layout, validate_runtime_layout, RuntimePaths};

fn main() {
    if let Err(error) = run() {
        eprintln!("phantomkernel-shardd failed to start: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    println!("phantomkernel-shardd initialized");
    Ok(())
}
